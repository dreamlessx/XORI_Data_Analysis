#!/usr/bin/env python3
"""
metric_calc_culled.py
Calculates metrics from tuning curve data with SNR-based culling.
Creates two types of culled outputs:
1. per_cull: Filters by top percentage of SNR (top_90, top_80, top_70)
2. thr_cull: Filters by SNR threshold (above_0.5, above_1.0, above_1.5)
...
def main():
    ...
    # Define culling criteria
    percentages = [90, 80, 70]  # top_90, top_80, top_70
    thresholds = [0.5, 1.0, 1.5]  # above_0.5, above_1.0, above_1.5
    
    # Create output directories
    base_output = 'metric_data/cull_roi'
    per_cull_dir = os.path.join(base_output, 'per_cull')
    thr_cull_dir = os.path.join(base_output, 'thr_cull')
    
    for perc in percentages:
        os.makedirs(os.path.join(per_cull_dir, f'top_{perc}'), exist_ok=True)
    
    for thr in thresholds:
        thr_str = str(thr).replace('.', '_')
        os.makedirs(os.path.join(thr_cull_dir, f'above_{thr_str}'), exist_ok=True)
    ...
    for site_name in site_folders:
        ...
        # Process percentage-based culling
        for perc in percentages:
            # Calculate how many ROIs to keep
            n_total = len(all_roi_data)
            n_keep = max(1, int(math.ceil((perc / 100.0) * n_total)))
            
            # Sort by SNR_g (descending) and take top N
            sorted_data = sorted(all_roi_data, key=lambda x: x[3], reverse=True)
            top_rois = sorted_data[:n_keep]
            
            # Re-sort by original ROI index for output
            top_rois_sorted = sorted(top_rois, key=lambda x: x[0])
            
            # Prepare output data (ROI index and metrics)
            output_data = [(roi[0], roi[1], roi[2], roi[3], roi[4], roi[5], roi[6], roi[7]) 
                          for roi in top_rois_sorted]
            
            # Write to file
            output_file = os.path.join(per_cull_dir, f'top_{perc}', f'metrics_{site_name}.txt')
            write_metrics_file(output_file, output_data)
            print(f"  top_{perc}: kept {len(top_rois)} ROIs")
        
        # Process threshold-based culling
        for thr in thresholds:
            # Filter ROIs by SNR threshold
            filtered_rois = [roi for roi in all_roi_data if roi[3] >= thr]
            
            if len(filtered_rois) == 0:
                print(f"  above_{thr}: no ROIs meet threshold (skipped)")
                continue
            ...
            # Write to file
            thr_str = str(thr).replace('.', '_')
            output_file = os.path.join(thr_cull_dir, f'above_{thr_str}', f'metrics_{site_name}.txt')
            write_metrics_file(output_file, output_data)
            print(f"  above_{thr}: kept {len(filtered_rois)} ROIs")
